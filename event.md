...
- [x] **nginx.conf /status 和 /metrics location blocks 結構錯誤**（本輪發現並修復）
  ...
  - metrics.lua 使用 module-level ngx.say，導致後續請求返回 5 bytes 空回應（module 只在第一次 require 時執行）
  - nginx.conf 缺少 `set $cont_upstream` 宣告，rewrite.lua 寫入時報 "variable not found for writing"
  - 修復：metrics.lua 重構為 function-based handler，nginx.conf 新增變數宣告與統一的 handler 呼叫 pattern
  - 已 commit `fc8ee4f4`，QA 驗證通過（/metrics 575 bytes 穩定，/status 200，40 Lua tests ✅）

- [x] **rewrite.lua / healthcheck.lua / worker.lua 使用 `require("init")`** — commit `7bb492ee`
...
- [x] **Proxy Lua Plugin 鏈完善化** — commit `270ea8b1` + `fb15630c` + `fc8ee4f4`
  - access.lua: JWT validation (validate_jwt), consumer auth (key-auth/basic-auth/hmac-auth via /internal/validate-cred), OPTIONS preflight, route matching, load balancing (roundrobin/leastconn/weighted-ip-hash)
  ...
  - **Bug fix `fc8ee4f4`**: metrics.lua module-level ngx.say bug → function-based handler；nginx.conf `set $cont_upstream` 宣告缺失修復
