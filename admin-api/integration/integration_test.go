...
func buildTestRouter() *gin.Engine {
	...
	svcs.GET("", routes.RequirePermission("services", false), routes.ListServices(testStore))
	svcs.POST("", routes.RequirePermission("services", true), routes.CreateService(testStore))
	svcs.GET("/:id", routes.RequirePermission("services", false), routes.GetService(testStore))
	svcs.PUT("/:id", routes.RequirePermission("services", true), routes.UpdateService(testStore))
	svcs.PATCH("/:id", routes.RequirePermission("services", true), routes.UpdateService(testStore))
	svcs.DELETE("/:id", routes.RequirePermission("services", true), routes.DeleteService(testStore))

	// Routes
	rt := admin.Group("/routes")
	rt.GET("", routes.RequirePermission("routes", false), routes.ListRoutes(testStore))
	rt.POST("", routes.RequirePermission("routes", true), routes.CreateRoute(testStore))
	rt.GET("/:id", routes.RequirePermission("routes", false), routes.GetRoute(testStore))
	rt.PUT("/:id", routes.RequirePermission("routes", true), routes.UpdateRoute(testStore))
	rt.PATCH("/:id", routes.RequirePermission("routes", true), routes.UpdateRoute(testStore))
	rt.DELETE("/:id", routes.RequirePermission("routes", true), routes.DeleteRoute(testStore))

	// Upstreams
	up := admin.Group("/upstreams")
	up.GET("", routes.RequirePermission("upstreams", false), routes.ListUpstreams(testStore))
	up.POST("", routes.RequirePermission("upstreams", true), routes.CreateUpstream(testStore))
	up.GET("/:id", routes.RequirePermission("upstreams", false), routes.GetUpstream(testStore))
	up.PUT("/:id", routes.RequirePermission("upstreams", true), routes.UpdateUpstream(testStore))
	up.PATCH("/:id", routes.RequirePermission("upstreams", true), routes.UpdateUpstream(testStore))
	up.DELETE("/:id", routes.RequirePermission("upstreams", true), routes.DeleteUpstream(testStore))
	up.GET("/:id/health", routes.RequirePermission("upstreams", false), routes.GetUpstreamHealth(testStore))
	up.GET("/:id/targets", routes.RequirePermission("targets", false), routes.ListTargets(testStore))
	up.POST("/:id/targets", routes.RequirePermission("targets", true), routes.CreateTarget(testStore))
	up.PUT("/:id/targets/:target_id", routes.RequirePermission("targets", true), routes.UpdateTarget(testStore))
	up.PATCH("/:id/targets/:target_id", routes.RequirePermission("targets", true), routes.UpdateTarget(testStore))
	up.DELETE("/:id/targets/:target_id", routes.RequirePermission("targets", true), routes.DeleteTarget(testStore))

	// Consumers
	cons := admin.Group("/consumers")
	cons.GET("", routes.RequirePermission("consumers", false), routes.ListConsumers(testStore))
	cons.POST("", routes.RequirePermission("consumers", true), routes.CreateConsumer(testStore))
	cons.GET("/:id", routes.RequirePermission("consumers", false), routes.GetConsumer(testStore))
	cons.PUT("/:id", routes.RequirePermission("consumers", true), routes.UpdateConsumer(testStore))
	cons.PATCH("/:id", routes.RequirePermission("consumers", true), routes.UpdateConsumer(testStore))
	cons.DELETE("/:id", routes.RequirePermission("consumers", true), routes.DeleteConsumer(testStore))

	// Consumer credentials
	cred := cons.Group("/:id")
	cred.GET("/key-auth/credentials", routes.RequirePermission("consumers", false), routes.ListCredentials(testStore, "key-auth"))
	cred.POST("/key-auth/credentials", routes.RequirePermission("consumers", true), routes.CreateCredential(testStore, "key-auth"))
	cred.DELETE("/key-auth/credentials/:credId", routes.RequirePermission("consumers", true), routes.DeleteCredential(testStore, "key-auth"))
	cred.GET("/basic-auth/credentials", routes.RequirePermission("consumers", false), routes.ListCredentials(testStore, "basic-auth"))
	cred.POST("/basic-auth/credentials", routes.RequirePermission("consumers", true), routes.CreateCredential(testStore, "basic-auth"))
	cred.DELETE("/basic-auth/credentials/:credId", routes.RequirePermission("consumers", true), routes.DeleteCredential(testStore, "basic-auth"))
	cred.GET("/hmac-auth/credentials", routes.RequirePermission("consumers", false), routes.ListCredentials(testStore, "hmac-auth"))
	cred.POST("/hmac-auth/credentials", routes.RequirePermission("consumers", true), routes.CreateCredential(testStore, "hmac-auth"))
	cred.DELETE("/hmac-auth/credentials/:credId", routes.RequirePermission("consumers", true), routes.DeleteCredential(testStore, "hmac-auth"))

	// Plugins
	plugs := admin.Group("/plugins")
	plugs.GET("", routes.RequirePermission("plugins", false), routes.ListPlugins(testStore))
	plugs.POST("", routes.RequirePermission("plugins", true), routes.CreatePlugin(testStore))
	plugs.GET("/:id", routes.RequirePermission("plugins", false), routes.GetPlugin(testStore))
	plugs.PUT("/:id", routes.RequirePermission("plugins", true), routes.UpdatePlugin(testStore))
	plugs.PATCH("/:id", routes.RequirePermission("plugins", true), routes.UpdatePlugin(testStore))
	plugs.DELETE("/:id", routes.RequirePermission("plugins", true), routes.DeletePlugin(testStore))

	// Workspaces
	ws := admin.Group("/workspaces")
	ws.GET("", routes.RequirePermission("workspaces", false), routes.ListWorkspaces(testStore))
	ws.POST("", routes.RequirePermission("workspaces", true), routes.CreateWorkspace(testStore))
	ws.GET("/:id", routes.RequirePermission("workspaces", false), routes.GetWorkspace(testStore))
	ws.PUT("/:id", routes.RequirePermission("workspaces", true), routes.UpdateWorkspace(testStore))
	ws.PATCH("/:id", routes.RequirePermission("workspaces", true), routes.UpdateWorkspace(testStore))
	ws.DELETE("/:id", routes.RequirePermission("workspaces", true), routes.DeleteWorkspace(testStore))
	ws.GET("/mine", routes.ListMyWorkspaces(testStore))
	ws.GET("/:id/users", routes.RequirePermission("workspaces", false), routes.ListWorkspaceUsers(testStore))
	ws.PUT("/:id/users", routes.RequirePermission("workspaces", true), routes.SetUserWorkspace(testStore))
	ws.DELETE("/:id/users/:userId", routes.RequirePermission("workspaces", true), routes.RemoveUserWorkspace(testStore))
	ws.GET("/users/:userId", routes.RequirePermission("workspaces", false), routes.GetUserWorkspaces(testStore))

	// Users
	users := admin.Group("/users")
	users.GET("", routes.RequirePermission("users", false), routes.ListUsers(testStore))
	users.POST("", routes.RequirePermission("users", true), routes.CreateUser(testStore))
	users.GET("/:id", routes.RequirePermission("users", false), routes.GetUser(testStore))
	users.PUT("/:id", routes.RequirePermission("users", true), routes.UpdateUser(testStore))
	users.PATCH("/:id", routes.RequirePermission("users", true), routes.UpdateUser(testStore))
	users.DELETE("/:id", routes.RequirePermission("users", true), routes.DeleteUser(testStore))

	// AuthGroups
	groups := admin.Group("/groups")
	groups.GET("", routes.RequirePermission("auth_groups", false), routes.ListAuthGroups(testStore))
	groups.POST("", routes.RequirePermission("auth_groups", true), routes.CreateAuthGroup(testStore))
	groups.GET("/:id", routes.RequirePermission("auth_groups", false), routes.GetAuthGroup(testStore))
	groups.PUT("/:id", routes.RequirePermission("auth_groups", true), routes.UpdateAuthGroup(testStore))
	groups.PATCH("/:id", routes.RequirePermission("auth_groups", true), routes.UpdateAuthGroup(testStore))
	groups.DELETE("/:id", routes.RequirePermission("auth_groups", true), routes.DeleteAuthGroup(testStore))
	groups.GET("/:id/members", routes.RequirePermission("auth_groups", false), routes.GetGroupMembers(testStore))
	groups.PUT("/:id/members", routes.RequirePermission("auth_groups", true), routes.SetGroupMembers(testStore))

	// API Key Requests
	keys := admin.Group("/api-keys")
	keys.GET("", routes.RequirePermission("api_keys", false), routes.ListAPIKeyRequests(testStore))
	keys.POST("", routes.RequirePermission("api_keys", true), routes.CreateAPIKeyRequest(testStore))
	keys.GET("/:id", routes.RequirePermission("api_keys", false), routes.GetAPIKeyRequest(testStore))
	keys.PUT("/:id", routes.RequirePermission("api_keys", true), routes.UpdateAPIKeyRequest(testStore))
	keys.PATCH("/:id", routes.RequirePermission("api_keys", true), routes.UpdateAPIKeyRequest(testStore))
	keys.DELETE("/:id", routes.RequirePermission("api_keys", true), routes.DeleteAPIKeyRequest(testStore))

	// Config Snapshots
	snaps := admin.Group("/config-snapshots")
	snaps.GET("", routes.RequirePermission("config_snapshots", false), routes.ListConfigSnapshots(testStore))
	snaps.POST("", routes.RequirePermission("config_snapshots", true), routes.CreateConfigSnapshot(testStore))
	snaps.DELETE("/:id", routes.RequirePermission("config_snapshots", true), routes.DeleteConfigSnapshot(testStore))

	// Audit Logs
	admin.GET("/audit", routes.RequirePermission("audit_logs", false), routes.ListAuditLogs(testStore))
