...
function _M.fetch_config()
    ...
    if not ok then
        ngx.log(ngx.ERR, "cont.config_sync: connect failed: ", err)
        return nil
    end
    ...
    if not bytes then
        ngx.log(ngx.ERR, "cont.config_sync: send failed: ", err)
        sock:close()
        return nil
    end
    ...
    if not headers then
        ngx.log(ngx.ERR, "cont.config_sync: receive headers failed: ", err)
        sock:close()
        return nil
    end
    ...
    if content_length then
        ...
        if not body then
            ngx.log(ngx.WARN, "cont.config_sync: receive body failed: ", err)
        end
    end
    ...
    if not parsed or not data then
        ngx.log(ngx.ERR, "cont.config_sync: JSON decode failed: ", data)
        return nil
    end
